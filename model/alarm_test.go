...
func TestAlarmProcessor(t *testing.T) {
	// Build and run processor
	tt := tester.New(t)
	group := new(EnhancedAlarm).DefineGroup("test", "alarms", "nodes", "enhanced")
	...
	alarm := &producer.Alarm{
		...
		ReductionKey: "uei.opennms.org/test::1",
		Description:  "This is a test",
		LogMessage:   "This is a test",
	}
	tt.Consume("alarms", "uei.opennms.org/test::1", alarm)
	// Verify enhanced alarm
	if _, value, ok := mt.Next(); ok {
		enhanced := value.(*EnhancedAlarm)
		assert.Assert(t, enhanced.Alarm != nil)
		assert.Equal(t, "This is a test", enhanced.Alarm.LogMessage)
